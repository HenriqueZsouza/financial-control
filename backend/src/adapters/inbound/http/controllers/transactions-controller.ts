...
export class TransactionsController {
  ...
  create = async (req: Request, res: Response, next: NextFunction) => { try {
    ...
    res.status(201).json({ transactions: (await this.createTransaction.execute(req.userId!, { ...data, ...(date ? { date: dateFromIso(date) } : {}) })).map(presentTransaction) });
  ...
  list = async (req: Request, res: Response, next: NextFunction) => { try {
    ...
    res.json({ transactions: query.scope === 'family' ? transactions.map(({ user, ...transaction }: any) => ({ ...presentTransaction(transaction), member: user })) : transactions.map(presentTransaction) });
  } catch (error) { next(error); } };
  getById = async (req: Request, res: Response, next: NextFunction) => { try { res.json({ transaction: presentTransaction(await this.getTransaction.execute(req.userId!, idParamSchema.parse(req.params.id))) }); } catch (error) { next(error); } };
  update = async (req: Request, res: Response, next: NextFunction) => { try {
    ...
    res.json({ transaction: presentTransaction(await this.updateTransaction.execute(req.userId!, idParamSchema.parse(req.params.id), { ...data, ...(date ? { date: dateFromIso(date) } : {}) })) });
  } catch (error) { next(error); } };
