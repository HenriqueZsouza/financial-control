...
import { createTransactionSchema, listTransactionsSchema, updateTransactionSchema } from '../dto/transaction-dto.js';

const categoryIds = (value: string | string[] | undefined) => (Array.isArray(value) ? value.flatMap((entry) => entry.split(',')) : value?.split(',') ?? []).map((id) => id.trim()).filter(Boolean);
export class TransactionsController {
  ...
  getById = async (req: Request, res: Response, next: NextFunction) => { try { res.json({ transaction: await this.getTransaction.execute(req.userId!, req.params.id) }); } catch (error) { next(error); } };
  update = async (req: Request, res: Response, next: NextFunction) => { try {
    ...
    res.json({ transaction: await this.updateTransaction.execute(req.userId!, req.params.id, { ...data, ...(date ? { date: dateFromIso(date) } : {}) }) });
  } catch (error) { next(error); } };
  remove = async (req: Request, res: Response, next: NextFunction) => { try { await this.deleteTransaction.execute(req.userId!, req.params.id); res.status(204).send(); } catch (error) { next(error); } };
}
