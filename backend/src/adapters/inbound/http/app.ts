...
import type { TelegramWebhookController } from './controllers/telegram-controller.js';
export function createHttpApp(controllers: HttpControllers, tokens: TokenIssuer, frontendOrigins: string[], swaggerEnabled = true, telegramWebhook?: TelegramWebhookController) {
  const app = express();
  ...
  if (telegramWebhook) app.post('/integrations/telegram/webhook', telegramWebhook.webhook);
