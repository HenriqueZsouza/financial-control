...
export interface HttpControllers { auth: AuthController; users: UsersController; categories: CategoriesController; transactions: TransactionsController; dashboard: DashboardController; family: FamilyController; notifications: NotificationsController }
export function apiRoutes(controllers: HttpControllers, tokens: TokenIssuer) {
  ...
  router.get('/family', protectedRoute, controllers.family.get); router.post('/family/invites', protectedRoute, controllers.family.createInvite); router.get('/family/invites/received', protectedRoute, controllers.family.listReceived); router.post('/family/invites/:id/accept', protectedRoute, controllers.family.acceptInvite); router.post('/family/invites/:id/decline', protectedRoute, controllers.family.declineInvite); router.delete('/family/members/:userId', protectedRoute, controllers.family.remove); router.post('/family/leave', protectedRoute, controllers.family.leave); router.post('/family/dissolve', protectedRoute, controllers.family.dissolve);
  router.get('/notifications', protectedRoute, controllers.notifications.list); router.post('/notifications/:id/read', protectedRoute, controllers.notifications.read); router.post('/notifications/read-all', protectedRoute, controllers.notifications.readAll);
  return router;
}
